// Server-side effective plan / paid-access helpers.
// Honors generic Apple/Wix plan_expiration_date AND isolated Google Trailblazer expiry.
// Never mutates provider-specific fields — read-only derivation.
 ...
 * expiration (open-ended subscription) or the expiration is still in the future.
 * Expired Trailblazer via plan_expiration_date no longer counts.
 */
export function isGenericPlanActive(user, now = new Date()) {
  ...
  // Subscriptions without an expiration remain active while status is active
  // or missing (legacy rows). Explicit past plan_expiration_date ends access.
  ...
  if (user.subscription_status === 'canceled' || user.subscription_status === 'expired') {
    // Canceled/expired without an explicit remaining expiration → no access
    return false;
  }
...
/**
 * Effective plan id considering both provider sources.
 * Higher tier wins when both are active (Trailblazer > Investigator > Explorer).
 */
...
/**
 * Which monthly energy pool is primary for spend/display.
 * Prefer isolated Google pool while the Google grant is active so refunds and
 * natural expiry cannot wipe Apple/Wix energy, and vice versa.
 */
...
/**
 * Monthly manifestation/narration balances for the active source, plus aura.
 * Does not merge Google + generic monthly pools into one number for spending —
 * callers that need spend order should use getSpendableEnergy.
 */
...
/**
 * Total spendable manifestation/narration across active monthly pool + aura.
 * When Google is active, Google monthly is tried first, then generic monthly
 * (if also active), then aura — so dual grants don't strand energy.
 */
...
/**
 * Compute field updates after spending 1 manifestation energy.
 * Order: google monthly → generic monthly → aura.
 */
...
/**
 * Compute field updates after spending `cost` narration energy.
 * Order: google monthly → generic monthly → aura.
 */
...
/** Best expiration date to show in UI (Google and/or generic). */
