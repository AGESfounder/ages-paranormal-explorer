...
Output ONLY a valid JSON object with a "new_stops" array.`;
  ...
  if (isLandmarkOrShip) {
    ...
    const newGeocodeMap = newStopsForGeocoding.length > 0 ? await geocodeStopsWithNames(newStopsForGeocoding, { lat: tour.start_latitude, lon: tour.start_longitude, maxDistMiles: tour.tour_category === 'road_trip' ? 200 : 5 }) : {};
