...
export default function CartDrawer() {
  const { cart, cartOpen, setCartOpen, removeFromCart, updateQuantity, cartTotal, checkout, checkoutLoading, checkoutError, shippingAddress, setShippingAddress } = useCart();

  const addrComplete = ['name', 'street', 'city', 'state', 'zip'].every(k => shippingAddress?.[k]);
          ...
          >
            ...
            {cart.length > 0 && (
              <div className="p-4 border-t border-border/40 space-y-3">
                ...
                <p className="text-[10px] text-muted-foreground">Shipping &amp; tax calculated at checkout based on your address.</p>
                <Button onClick={checkout} disabled={checkoutLoading || !addrComplete} className="w-full gap-2">
                  {checkoutLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <CreditCard className="w-4 h-4" />}
                  {checkoutLoading ? <BePatient /> : (addrComplete ? 'Pay with Card' : 'Enter shipping address')}
                </Button>
              </div>
            )}
          </motion.div>
