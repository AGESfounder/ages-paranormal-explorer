...
export default function ProductListItem({ product, onAdd, index }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="flex flex-col gap-3 p-3 rounded-xl border border-border/40 bg-card/40">
      <div className="flex gap-3">
        <div className="w-20 h-20 rounded-lg bg-secondary/30 overflow-hidden shrink-0">
          {product.image_url ? <img src={product.image_url} alt={product.name} className="w-full h-full object-contain" /> : null}
        </div>
      ...
      {product.video_url && (
        <div className="space-y-1">
          <p className="text-[9px] text-muted-foreground uppercase tracking-wider">Video</p>
          <video src={product.video_url} controls className="w-full max-h-40 rounded-md bg-black object-contain" />
        </div>
      )}
    </motion.div>
  );
